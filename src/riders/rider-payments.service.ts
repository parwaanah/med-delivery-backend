...
export class RiderPaymentsService {
  private readonly logger = new Logger(RiderPaymentsService.name);
  private readonly baseFare: number;
  private readonly perKm: number;
  private readonly bonusPerOrder: number;
  private readonly cancellationPenalty: number;
  ...
  constructor(
    ...
    this.cancellationPenalty = Number(
      this.config.get('RIDER_CANCELLATION_PENALTY') ?? 25,
    );
  }
  ...
  private async computeDistanceKm(orderId: number) {
  ...
  private hasDeliveryProof(order: {
  ...
  // Rider paid only after delivery + proof exists.
  async ensureDeliveryEarningForOrder(orderId: number) {
    const order: any = await this.prisma.order.findUnique(({
      where: { id: orderId },
      select: {
        id: true,
    ...
    if (!this.hasDeliveryProof(order)) return null;
    ...
    const surgeStatus = await this.surge
      .getStatus()
      .catch(() => ({ multiplier: 1 }));
    const multiplier = Number((surgeStatus as any)?.multiplier ?? 1) || 1;
    ...
    return (this.prisma as any).riderEarning.upsert({
      where: { orderId },
      create: {
        orderId,
  ...
  // Refund split: claw back rider payout if not settled.
  async handleRefundForOrder(
    orderId: number,
    opts?: { transactionId?: string; amount?: number; by?: string },
  ) {
    const earning: any = await (this.prisma as any).riderEarning.findUnique({
      where: { orderId },
    });
    if (!earning) return { ok: true, changed: false, missing: true };

    const status = String(earning.status || '').toUpperCase();
    if (status === 'SETTLED') {
      this.logger.warn(
        `Refund clawback skipped: rider earning already settled (orderId=${orderId})`,
      );
    ...
    const updated = await (this.prisma as any).riderEarning.update({
      where: { orderId },
      data: {
        // Keep breakdown for audit; make the payout zero.
        netAmount: this.d(0),
        meta: nextMeta,
      },
    });
  ...
  async applyCancellationPenaltyForOrder(
    orderId: number,
    riderId: number,
    reason?: string,
  ) {
    ...
    const existing: any = await (this.prisma as any).riderEarning.findUnique({
      where: { orderId },
    });

    if (!existing) {
      return (this.prisma as any).riderEarning.create({
        data: {
          orderId,
          riderId,
          type: 'PENALTY',
    ...
    const nextMeta = {
      ...(existing.meta || {}),
      penalties: [
        ...(((existing.meta || {}) as any)?.penalties || []),
        { at: new Date().toISOString(), reason, amount: penalty },
      ],
    };

    return (this.prisma as any).riderEarning.update({
      where: { orderId },
      data: {
        riderId,
  ...
  async getSummary(riderId: number) {
    ...
    const [earned, pendingAgg, last7days] = await Promise.all([
      (this.prisma as any).riderEarning.aggregate({
  ...
  async getTransactions(riderId: number, query: TransactionsQuery = {}) {
    const limit = Math.min(200, Math.max(1, query.limit ?? 50));

    const where: any = { riderId };
    if (query.status) where.status = String(query.status).toUpperCase();

    const rows: any[] = await (this.prisma as any).riderEarning.findMany({
      where,
  ...
  async createWeeklyBatch(periodStart: Date, periodEnd: Date, createdBy?: number) {
    const existing = await (this.prisma as any).riderSettlementBatch.findUnique({
      where: { periodStart_periodEnd: { periodStart, periodEnd } } as any,
    });
    if (existing) return existing;

    const batch = await (this.prisma as any).riderSettlementBatch.create({
      data: {
        periodStart,
        periodEnd,
        status: 'CREATED',
    ...
    const res = await (this.prisma as any).riderEarning.updateMany({
      where: {
    ...
    this.logger.log(
      `Created rider settlement batch #${batch.id} (${res.count} earnings)`,
    );

    if (createdBy) {
      await (this.prisma as any).riderSettlementBatch.update({
        where: { id: batch.id },
        data: {
          status: batch.status,
          // no meta column in schema; audit via AdminAudit + logs only
        },
      });
    }
  ...
  async markBatchPaid(batchId: number, paidBy?: number) {
    const now = new Date();

    const batch = await (this.prisma as any).riderSettlementBatch.update({
      where: { id: batchId },
      data: { status: 'PAID' },
    });

    await (this.prisma as any).riderEarning.updateMany({
  ...
  async adminOverrideEarning(
    earningId: number,
    patch: { bonus?: number; penalty?: number },
  ) {
    const current: any = await (this.prisma as any).riderEarning.findUnique({
      where: { id: earningId },
    });
    ...
    const penalty =
      patch.penalty != null ? this.d(Math.abs(patch.penalty)) : current.penalty;

    const net = new Prisma.Decimal(0)
